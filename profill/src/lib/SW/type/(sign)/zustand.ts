...
export const store = create< statesType & methodType >()(
  persist((set) => ({
    ...
    incrementDEX: (A,B) => set({DEX:A+B}),
    incrementAGI: (C,B) => set({AGI:C+B}),
    incrementINT: (C,D) => set({INT:C+D}),
    incrementSTR: (E,F) => set({STR:E+F}),
    incrementHP: (G,F) => set({HP:G+F}),
    incrementSP: (G,H) => set({SP:G+H}),
