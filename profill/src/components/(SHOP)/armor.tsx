...
const Armor = ({armors1, armors2}:{armors1:armor1Type, armors2:armor2Type}) => {
  ...
  <div className="relative w-[594px] h-[420px] bg-white m-4 flex-col justify-center items-center border">
    <div className='h-[40px] flex items-center'>
      {/* 防具の名前 */}
      <div className='flex w-[280px] h-[40px] pl-5 items-center border'>名称（防具ー鎧、盾）</div>
      {/* 必要筋力 */}
      <div className='flex w-[140px] h-[40px] pl-4 items-center border'>必要筋力の範囲</div>
      {/* 値 */}
      <div className='flex w-[70px] h-[40px] pl-1 justify-center items-center border'>筋力値</div>
      {/* 値段 */}
    ...
    <div className='w-full h-[40px] flex items-center pl-4 '>非金属鎧</div>
      ...
      <div key={armor.id}>
        <form action={formAction} className='flex'>
          {/* 防具の名前 */}
          <input name="itemName" defaultValue={armor.name} className='flex w-[280px] h-[25px] pl-5 items-center border ' readOnly />
          {/* 必要筋力 */}
          ...
          {/* 値 */}
          <input type='hidden' name="itemStr" defaultValue={state[index]} readOnly /><div className='flex justify-center w-[30px] h-[25px] border bg-white'>{state[index]}</div>
          {/* +ボタン */}
          <button type='button' onClick={() => handleClickAdd(index,armor.max)} className='flex w-[20px] h-[25px] px-1 justify-center items-center border hover:bg-slate-300' >+</button>
          {/* 値段 */}
          <input type='hidden' name="itemPrice" defaultValue={state[index]*armor.multiplication+armor.addition} readOnly /><div className='flex justify-center w-[50px] h-[25px] border bg-white'>{state[index]*armor.multiplication+armor.addition}</div>
          {/* 買うボタン */}
    ...
    <div className='h-[40px] flex items-center pl-4 '>金属鎧</div>
      ...
      <div key={armor.id}>
        <form action={formAction} className='flex justify-center items-center '>
          ...
          {/* 値 */}
          <input type='hidden' name='itemStr' defaultValue={stateArmor2[index]} readOnly /><div className='flex justify-center w-[30px] h-[25px] border bg-white'>{stateArmor2[index]}</div>
          {/* +ボタン */}
          ...
          {/* 値段 */}
          <input type='hidden' name='itemPrice' defaultValue={stateArmor2[index]*armor.multiplication+armor.addition} readOnly /><div className='flex justify-center w-[50px] h-[25px] border bg-white'>{stateArmor2[index]*armor.multiplication+armor.addition}</div>
          {/* 買うボタン */}
          <button type='submit' className={`flex w-[54px] h-[25px] justify-center items-center border hover:bg-slate-300 '}`}>買う</button>
        </form>
      </div>
    ...
    <div>
      <form action={formActionShield} className="flex justify-between">
        {/* 防具の名前 */}
        <input name="itemName" defaultValue="スモールシールド" className='flex w-[280px] h-[25px] pl-5 items-center border' readOnly />
        {/* 必要筋力 */}
    ...
    <div >
      ...
      <form action={formActionShield} className="flex justify-between items-center">
        <input name="itemName" defaultValue="ラージシールド" className='flex w-[280px] h-[25px] pl-5 items-center border bg-white' readOnly />
        {/* 必要筋力 */}
